  ...
  try {
    ...
    if (!summarizeContentType.includes("application/json")) {
      return res.status(500).json({
        error: `Summarize endpoint did not return JSON: ${summarizeRaw}`
      });
    }

    const summarizeData = JSON.parse(summarizeRaw);

    if (!summarizeResponse.ok) {
      return res.status(500).json({
        error: summarizeData.error || "Error generating summary"
      });
    }
